      ...
      <React.Fragment>
        {/* <div className="dropdown"> */}
          ...
          <div className="dropdown-menu"> */}
            ...
            {abiTable.function && abiTable.function.filter(method => method.stateMutability !== 'view').map(method => ( */}
              <button
                key={`function-${abiTable.function[12].name}`}
                className="btn btn-secondary"
                onClick={() => this.onSelect(abiTable.function[12])}
              >
                {abiTable.function[12].name}
              </button>
            {/* ))} */}
            {/* <div className="dropdown-divider"></div>
  ...
  onSubmit = async (selected, params) => {
    if (selected.type === 'event') {
      ...
      try {
        ...
        this.setState({ loading: false, result: JSON.stringify(result), error: '' })
      } catch (err) {
        this.setState({ loading: false, result: '', error: err.message })
        return
      }
